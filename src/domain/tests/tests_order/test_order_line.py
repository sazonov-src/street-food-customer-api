...
def test_total_price(item):
    line = order.CartItem(item, order.Count(5)) 
...
def test_bed_minus_count(item):
    line = order.CartItem(item, order.Count(1)) 
    with pytest.raises(ValueError):
        line.minus_count()

def test_eq_orderline(item):
    line1 = order.CartItem(item, order.Count(1)) 
    line2 = order.CartItem(item, order.Count(5)) 
