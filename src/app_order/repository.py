...
from domain.logic import order_state
...
def get_new_order(user):
    ...
    for order_ in order_set:
        ...
        if order_state_ := order_state.get_new_order_handler_or_false(order.Order(**serializer.data)):
            return order_, order_state_ 
    return None, None


def create_new_order_obj(user):
    order_handler = order_state.NewOrderHandler()
