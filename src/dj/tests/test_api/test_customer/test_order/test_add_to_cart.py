...
@pytest.mark.django_db
def test_add_to_order_cart_bed_count(client, user):
    mixer.blend("app_menu.MenuItem", id=1)
    r: Response = client.post("/api/v1/customer/new-order/cart/", {"id_menu_item":1, "count": -1})
    assert r.status_code == status.HTTP_400_BAD_REQUEST
    r: Response = client.post("/api/v1/customer/new-order/cart/", {"id_menu_item":1, "count": 0})
    assert r.status_code == status.HTTP_400_BAD_REQUEST
    r: Response = client.post("/api/v1/customer/new-order/cart/", {"id_menu_item":1, "count": "pr"})
    assert r.status_code == status.HTTP_400_BAD_REQUEST
